...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const storyIntroSection = document.getElementById('envelopeSection');
  ...
  function loadPhotosFromManifest(files) {
    // 9 foto pertama dipasang ke slot polaroid + cerita, sisanya masuk ke galeri.
    ...
    // Galeri: dibuat lazy, browser cuma download foto pas mau kelihatan di layar,
    // bukan langsung semua foto sekaligus pas halaman dibuka.
    files.forEach((filename, i) => {
      addGalleryItem('images/' + filename, i + 1);
    });
  }
  ...
  function addGalleryItem(src, index) {
    ...
    img.decoding = 'async';
    img.onerror = () => {
      // Foto di daftar tapi filenya tidak ketemu / gagal dimuat, lewati otomatis
  ...
  function addVideoCard(src) {
    ...
    // Tap video: nyalain suara video, matiin musik latar
    ...
    // Video baru mulai didownload begitu hampir masuk layar (lazy load),
    // bukan langsung semua video sekaligus pas halaman dibuka.
  ...
  if (calendarGrid) {
    ...
    // 1 Juli 2026 jatuh pada hari Rabu -> index 2 (Senin=0)
  ...
  const letterRevealWrapper = document.getElementById('letterRevealWrapper');

  function openEnvelope() {
    envelopeGraphic.classList.add('opening');
    setTimeout(() => {
      envelopeTeaser.classList.add('hidden-away');
      letterRevealWrapper.classList.add('revealed');
      letterRevealWrapper.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 650);
  }
